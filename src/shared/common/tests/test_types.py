...
class TestTypedIdentifiers:
    ...
    @allure.title("Typed IDs generate unique values, support equality, and are hashable")
    def test_uniqueness_equality_and_hashability(self):
        for id_cls in (CodeId, SegmentId, CaseId, FolderId):
            id1 = id_cls.new()
            id2 = id_cls.new()
            assert id1.value != id2.value, f"{id_cls.__name__} did not generate unique IDs"
...
class TestFailureReasons:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("DuplicateName", lambda: DuplicateName(name="Theme"), lambda f: "Theme" in f.message and f.message == "Code name 'Theme' already exists"),
            ("CodeNotFound", lambda: CodeNotFound(code_id=CodeId(value="42")), lambda f: f.message == "Code with id 42 not found"),
            ("SourceNotFound", lambda: SourceNotFound(source_id=SourceId(value="99")), lambda f: f.message == "Source with id 99 not found"),
            ("InvalidPosition", lambda: InvalidPosition(start=100, end=200, source_length=50), lambda f: "Position [100:200]" in f.message and "length 50" in f.message),
            ("EmptyName", lambda: EmptyName(), lambda f: f.message == "Code name cannot be empty"),
            ("FolderNotFound", lambda: FolderNotFound(folder_id=FolderId(value="7")), lambda f: f.message == "Folder with id 7 not found"),
        ],
        ids=["duplicate_name", "code_not_found", "source_not_found", "invalid_position", "empty_name", "folder_not_found"],
    )
