...
class TestImportTextDocument:
    ...
    def test_supported_formats_and_extraction(
        ...
        with allure.step("Verify TextExtractor supports .txt, .docx, .rtf, .odt, .md"):
        ...
        with allure.step("Verify extraction succeeded with expected content"):
            assert isinstance(result, Success)
            data = result.unwrap()
            assert "Interview Transcript" in data.content
            assert "time management" in data.content.lower()

        with allure.step("Verify metadata sufficient for source list display"):
            assert data.file_size > 0
            assert len(data.content) > 0
            assert data.encoding is not None

    @allure.title("AC #4: Formatting preserved, plus Unicode content handled")
    def test_formatting_preserved_and_unicode(
        self, text_extractor, sample_files: SampleFiles
    ):
        from returns.result import Success
        ...
        with allure.step("Verify paragraph breaks and indentation preserved"):
            assert isinstance(result, Success)
...
class TestImportPdfDocument:
    ...
    @allure.title("AC #1+2: Select PDF files and extract text from pages")
    def test_select_and_extract_pdf(self, pdf_extractor, sample_files: SampleFiles):
        from returns.result import Success

        with allure.step("Verify PdfExtractor supports .pdf and rejects others"):
    ...
    @allure.severity(allure.severity_level.NORMAL)
    def test_multipage_pdf_handled(self, pdf_extractor):
        ...
        with allure.step("Verify page count and content from all pages"):
...
class TestImportImageFiles:
    ...
    @allure.title("AC #1+3: Select and import multiple image files")
    def test_select_and_import_multiple_images(self, sample_files: SampleFiles):
    ...
    @allure.title("AC #2+4: Images displayed in viewer with metadata")
    def test_images_displayed_with_metadata(
        ...
        with allure.step("Get and verify metadata"):
        ...
        attach_screenshot(viewer, "ImageViewer - Loaded Image with Metadata")
...
class TestImportAudioVideoFiles:
    ...
    @allure.title("AC #1+3: Select audio files and verify duration display")
    def test_select_audio_and_duration(self, qapp, colors, sample_files: SampleFiles):
        from src.shared.presentation.organisms import MediaPlayer

        with allure.step("Verify audio files exist"):
    ...
    @allure.title("Backend detection and signal emissions")
    @allure.severity(allure.severity_level.NORMAL)
    def test_backend_detection_and_signals(
        self, qapp, colors, sample_files: SampleFiles
    ):
        from src.shared.presentation.organisms import MediaPlayer

        with allure.step("Create MediaPlayer and verify backend"):
            player = MediaPlayer(colors=colors)
        ...
        with allure.step("Set up signal spies"):
...
class TestOrganizeSources:
    ...
    @allure.title("AC #4: Folder structure, dialog validation, and entity operations")
    def test_folder_structure_dialog_and_entity(self, qapp):
        ...
        with allure.step("Create FolderTree widget with hierarchy"):
            tree = FolderTree()
            folders = [
        ...
        with allure.step("Verify FolderDialog validates input"):
            dialog = FolderDialog()
            assert dialog.folder_name == ""

        with allure.step("Test folder entity rename and move"):
            folder = Folder(id=FolderId("1"), name="Test Folder", parent_id=None)
    ...
    def test_folder_operations_persist(self, tmp_path):
        ...
        with allure.step("Step 4: Verify folder and source in folder persisted"):
...
class TestViewSourceMetadata:
    ...
    @allure.title("AC #1+4: See file info and edit source properties")
    def test_file_info_and_edit_properties(self, sample_files: SampleFiles):
        from src.contexts.projects.core.entities import Source, SourceType
        from src.shared.common.types import SourceId

        with allure.step("Get sample text file and verify metadata"):
        ...
        with allure.step("Create and rename source entity"):
            source = Source(
                id=SourceId("1"),
                name="old_name.txt",
                source_type=SourceType.TEXT,
    ...
    @allure.title("AC #2+3: Add memo and view coding statistics")
    def test_memo_and_coding_statistics(self):
        from src.contexts.projects.core.entities import Source, SourceType
        ...
        with allure.step("Create source and add memo"):
            source = Source(
                id=SourceId("1"),
                name="test.txt",
                source_type=SourceType.TEXT,
        ...
        with allure.step("Verify coding statistics in metadata"):
...
class TestMediaPlayerUIControls:
    ...
    @allure.title("Play button toggles playback and emits signals")
    def test_play_button_toggle_and_signals(
        ...
        with allure.step("Set up signal spy and click play"):
            started_spy = QSignalSpy(player.playback_started)
            qtbot.mouseClick(player._play_btn, Qt.MouseButton.LeftButton)
            QApplication.processEvents()

        with allure.step("Verify playing and signal emitted"):
            if player.get_backend_name() == "Noop":
                assert player.is_playing()
            assert started_spy.count() == 1
    ...
    @allure.title("Volume slider and progress slider control playback")
    def test_volume_and_progress_sliders(
        self, qtbot, qapp, colors, sample_files: SampleFiles
    ):
        from src.shared.presentation.organisms import MediaPlayer
        ...
        with allure.step("Simulate progress slider seek"):
            player._progress_slider.setValue(500)
            player._on_seek(500)
            QApplication.processEvents()
...
class TestImageViewerUIControls:
    ...
    @allure.title("Fit and actual size buttons control display mode")
    def test_fit_and_actual_size_buttons(
        self, qtbot, qapp, colors, sample_files: SampleFiles
    ):
        from PySide6.QtCore import Qt
        ...
        attach_screenshot(viewer, "ImageViewer - Fit and Actual Size")

    @allure.title("Zoom methods change level and update label")
    def test_zoom_methods_and_label(
        self, qtbot, qapp, colors, sample_files: SampleFiles
    ):
        from src.shared.presentation.organisms import ImageViewer
        ...
        with allure.step("Zoom in and verify level and label changed"):
            viewer.zoom_in()
            QApplication.processEvents()
        ...
        with allure.step("Zoom out and verify back to original"):
            viewer.zoom_out()
            QApplication.processEvents()
            assert viewer.get_zoom_level() == initial_zoom
...
class TestAgentListSources:
    ...
    @allure.title("AC #1+2: List all sources with IDs and filter by type")
    def test_list_sources_and_filter_by_type(self, app_context, project_with_sources):
        from returns.result import Success
        ...
        with allure.step("Execute list_sources and verify all returned with IDs"):
...
class TestAgentReadSourceContent:
    ...
    @allure.title("AC #1+2+3: Get text content and read by position range with markers")
    def test_get_content_and_position_range(self, app_context, project_with_content):
        from returns.result import Success
        ...
        with allure.step("Read full source content"):
    ...
    def test_large_sources_paginated(self, app_context, project_with_content):
        from returns.result import Success
...
class TestAgentExtractMetadata:
    ...
    @allure.title("AC #1+2: Detect language and extract key topics")
    def test_language_detection_and_topics(self, app_context, project_with_source):
        from returns.result import Success
    ...
    @allure.title("AC #3+4: Organization suggestion and approval required")
    def test_organization_and_approval(self, app_context, project_with_source):
        from returns.result import Success
        ...
        with allure.step("Submit metadata and verify pending approval"):
            result = tools.execute(
                "suggest_source_metadata",
                {
                    "source_id": 1,
