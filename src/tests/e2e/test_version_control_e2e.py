...
class TestVersionHistoryScreen:
    ...
    def test_screen_displays_snapshot_cards_and_refresh(self, qapp, colors, version_history_screen):
        ...
        with allure.step("Verify refresh signal connection works"):
            signal_emitted = []
...
class TestDiffViewerDialog:
    ...
    @allure.title("AC #4: Dialog displays diff content, refs, and correct stats")
    def test_dialog_displays_diff_content_refs_and_stats(self, diff_viewer_dialog):
        """E2E: Dialog shows diff with syntax highlighting, commit refs, and correct stats."""
        ...
        attach_screenshot(diff_viewer_dialog, "DiffViewerDialog - Full View")
...
class TestRestoreSnapshot:
    ...
    @allure.title("AC #5: Restore button visibility and signal emission")
    def test_restore_button_visibility_and_signal(self, qapp, colors):
        """E2E: Restore button appears only on non-current snapshots and emits signal with SHA."""
        ...
        with allure.step("Set up signal spy and click restore"):
