...
MCP tools: import_file (NOTIFY), rename_source (NOTIFY), delete_source (REQUIRE), detect_speakers (SUGGEST), import_survey (REQUIRE). Register with MCP server.
<!-- SECTION:DESCRIPTION:END -->

## Acceptance Criteria
<!-- AC:BEGIN -->
### MCP Tool Schemas (following MCP Tool Schema Protocol)
...
### Schema Structure (per MCP protocol)
- [ ] #9 Each schema defines: name, description, inputSchema (JSON Schema), trust_level
- [ ] #10 Schemas registered with MCP server on startup
- [ ] #11 Resource URIs: `qualcoder://sources/tools/{tool_name}`

### Controller Integration
- [ ] #12 Each MCP tool invokes corresponding Controller command
- [ ] #13 Controller returns Result[Event, Failure] → MCP returns success/error
