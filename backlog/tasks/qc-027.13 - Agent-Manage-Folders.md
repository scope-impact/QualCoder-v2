...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 MCP tool `list_folders` returns all folders with id, name, parent_id hierarchy
- [ ] #2 MCP tool `create_folder` accepts name and optional parent_id, returns new folder ID
- [ ] #3 Duplicate folder names within same parent are rejected with clear failure message
- [ ] #4 MCP tool `rename_folder` accepts folder_id and new_name
- [ ] #5 MCP tool `delete_folder` accepts folder_id, rejects if folder contains sources
- [ ] #6 MCP tool `move_source_to_folder` accepts source_id and target folder_id
- [ ] #7 All tools publish corresponding domain events (FolderCreated, FolderRenamed, FolderDeleted, SourceMovedToFolder)
- [ ] #8 E2E test exists with `@allure.story("QC-027.13 Agent Manage Folders")` decorator
<!-- AC:END -->
