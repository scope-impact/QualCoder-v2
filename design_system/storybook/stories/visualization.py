...
from ...charts import ChartDataPoint, ChartWidget, PieChart, SparkLine
from ...data_display import HeatMapCell, HeatMapGrid
from ...image_annotation import AnnotationMode, ImageAnnotationLayer
from ...network_graph import GraphEdge, GraphNode, NetworkGraphWidget
...
def create_charts_story(colors: ColorPalette) -> StoryPage:
    ...
    examples.append(
        ("Donut Chart", donut_chart, 'PieChart(title="Coverage", donut=True)')
    )
    ...
    spark2 = SparkLine(
        values=[30, 28, 25, 22, 20, 18, 15, 12], color=colors.error, colors=colors
    )
    spark3 = SparkLine(
        values=[15, 20, 18, 25, 22, 30, 28, 35], color=colors.success, colors=colors
    )
    ...
    examples.append(
        ("Sparklines", container, "SparkLine(values=[10, 15, 12, 18, ...])")
    )

    return StoryPage(
        "Charts", "Data visualization charts using PyQtGraph.", examples, colors=colors
    )
...
def create_word_cloud_story(colors: ColorPalette) -> StoryPage:
    ...
    examples.append(
        ("Word Cloud (Rainbow)", wc_rainbow, 'WordCloudWidget(color_scheme="rainbow")')
    )
...
def create_heatmap_story(colors: ColorPalette) -> StoryPage:
    ...
    examples.append(
        ("Heat Map Cells", container, 'HeatMapCell(value=0.75, color_scheme="primary")')
    )
...
def create_score_bars_story(colors: ColorPalette) -> StoryPage:
    ...
    for score, label in [
        ...
        bar = RelevanceScoreBar(
            score=score, label=label, variant="confidence", colors=colors
        )
    ...
    layout3.addWidget(
        ScoreIndicator(
            label="Confidence", score=0.72, variant="confidence", colors=colors
        )
    )
    layout3.addWidget(
        ScoreIndicator(label="Match", score=0.95, variant="match", colors=colors)
    )
    layout3.addStretch()
...
def create_pdf_viewer_story(colors: ColorPalette) -> StoryPage:
    ...
    viewer_thumbs = PDFPageViewer(
        show_toolbar=True, show_thumbnails=True, colors=colors
    )
