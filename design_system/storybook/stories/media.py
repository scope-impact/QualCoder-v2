...
from ...media import PlayerControls, ThumbnailStrip, Timeline, VideoContainer
...
def create_player_story(colors: ColorPalette) -> StoryPage:
    ...
    examples.append(("Video Container", video, "VideoContainer(aspect_ratio=16/9)"))
...
def create_upload_story(colors: ColorPalette) -> StoryPage:
    ...
    examples.append(
        ("Drop Zone", drop, 'DropZone(accepted_types=["text", "audio"], max_files=10)')
    )
...
def create_calendar_story(colors: ColorPalette) -> StoryPage:
    ...
    examples.append(("Mini Calendar", calendar, "CalendarMini()"))

    # Date range picker
    date_range = DateRangePicker(colors=colors)
    examples.append(("Date Range Picker", date_range, "DateRangePicker()"))

    # Quick date select
    quick = QuickDateSelect(colors=colors)
    examples.append(("Quick Date Select", quick, "QuickDateSelect()"))

    return StoryPage(
        "Calendar", "Date selection and calendar components.", examples, colors=colors
    )
