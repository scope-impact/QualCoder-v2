...
from ...chat import ChatInput, MessageBubble, QuickPrompts
...
def create_messages_story(colors: ColorPalette) -> StoryPage:
    ...
    examples.append(("User Message", user_msg, 'MessageBubble("Hello!", role="user")'))
    ...
    examples.append(
        ("Assistant Message", ai_msg, 'MessageBubble("Response...", role="assistant")')
    )

    # Quick prompts
    prompts = QuickPrompts(
        ["Suggest codes", "Summarize text", "Find similar"], colors=colors
    )
    examples.append(
        ("Quick Prompts", prompts, 'QuickPrompts(["Suggest codes", "Summarize text"])')
    )
...
def create_chatinput_story(colors: ColorPalette) -> StoryPage:
    ...
    examples.append(
        ("Chat Input", chat, 'ChatInput(placeholder="Ask AI assistant...")')
    )

    return StoryPage(
        "Chat Input", "Input component for chat interfaces.", examples, colors=colors
    )
