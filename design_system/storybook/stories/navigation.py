...
from ...navigation import Breadcrumb, StepIndicator, TabGroup
...
def create_steps_story(colors: ColorPalette) -> StoryPage:
    ...
    steps = StepIndicator(
        ["Select Files", "Configure", "Process", "Review"], current=2, colors=colors
    )
    examples.append(
        (
            "Step Indicator",
...
def create_pagination_story(colors: ColorPalette) -> StoryPage:
    ...
    examples.append(
        ("Pagination", pagination, "Pagination(total_pages=10, current_page=3)")
    )

    # Simple pagination
    simple = SimplePagination(current=5, total=20, colors=colors)
    examples.append(
        ("Simple Pagination", simple, "SimplePagination(current=5, total=20)")
    )
